...
pub mod aes {
    ...
    fn round_constant_i(i: u8) -> u32 {
        fn round_constant_i_top_byte(i: u8) -> u8 {
            match i {
                0 => 1,
                _ if round_constant_i_top_byte(i - 1) < 0x80 => round_constant_i_top_byte(i - 1).wrapping_mul(2),
    ...
    fn key_expansion(key: &[u8; 16], round_count: u8) -> Vec<Vec<u8>> {
        ...
        for i in (0 ..key.len()).step_by(4) {
            key_in_32bit_words.push(u32::from_be_bytes(*arrayref::array_ref!(key[i .. i + 4], 0, 4)));
        }
        for i in 0 .. round_keys_u32.len() {
            round_keys_u32.push(
                match i {
                    i if i < N => key_in_32bit_words[i],
                    i if i >= N && (i % N == 0) => round_keys_u32[i - N] ^ sub_word(round_keys_u32[i - 1]).rotate_left(1) ^ round_constants[i / N],
                    i if i >= N && N > 6 && (i % N == 4) => round_keys_u32[i - N] ^ sub_word(round_keys_u32[i - 1]),
    ...
    fn sub_word(word: u32) -> u32 {
        let subbed_bytes_vec = word.to_be_bytes().iter().map(|x| s_box(*x)).collect::<Vec<u8>>();
